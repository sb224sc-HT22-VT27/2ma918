...
def solve_exercise_1_1():
    ...
    # Nodes: Almhult(0), Markaryd(1), Liatorp(2), Osby(3), Ljungby(4), Alvesta(5), Vaxjo(6)
    
    # Edges and their properties: (from, to, cost, capacity)
    edges = [
        (0, 2, 18, 1000),  # Almhult -> Liatorp
        (0, 3, 24, 1000),  # Almhult -> Osby
        (0, 6, 62, 1000),  # Almhult -> Vaxjo
        (3, 1, 29, 1000),  # Osby -> Markaryd
        (2, 4, 31, 500),   # Liatorp -> Ljungby
        (2, 6, 46, 1000),  # Liatorp -> Vaxjo
        (1, 4, 51, 2000),  # Markaryd -> Ljungby
        (4, 5, 42, 500),   # Ljungby -> Alvesta
        (5, 6, 20, 2000),  # Alvesta -> Vaxjo
    ]
    ...
    # Supply: Almhult=2500, Markaryd=1000
    # Demand: Ljungby=1000, Alvesta=500, Vaxjo=2000
    # Transshipment: Liatorp=0, Osby=0
    
    # A_eq matrix for flow conservation (rows = nodes, cols = edges)
    # Node order: Almhult(0), Markaryd(1), Liatorp(2), Osby(3), Ljungby(4), Alvesta(5), Vaxjo(6)
    num_nodes = 7
    ...
    for i, (from_node, to_node, cost, capacity) in enumerate(edges):
        node_names = ['Almhult', 'Markaryd', 'Liatorp', 'Osby', 'Ljungby', 'Alvesta', 'Vaxjo']
        if result.x[i] > 0.001:  # Only show non-zero flows
            print(f"  {node_names[from_node]:10s} -> {node_names[to_node]:10s}: {result.x[i]:8.2f} units (cost: {cost}, capacity: {capacity})")
...
def solve_exercise_1_2_varnamo():
    ...
    # Nodes: Almhult(0), Markaryd(1), Varnamo(2), Liatorp(3), Osby(4), Ljungby(5), Alvesta(6), Vaxjo(7)
    
    # Updated edges with Varnamo
    edges = [
        (0, 3, 18, 1000),  # Almhult -> Liatorp
        (0, 4, 24, 1000),  # Almhult -> Osby
        (0, 7, 62, 1000),  # Almhult -> Vaxjo
        (4, 1, 29, 1000),  # Osby -> Markaryd
        (3, 5, 31, 500),   # Liatorp -> Ljungby
        (3, 7, 46, 1000),  # Liatorp -> Vaxjo
        (1, 5, 51, 2000),  # Markaryd -> Ljungby
        (5, 6, 42, 500),   # Ljungby -> Alvesta
        (6, 7, 20, 2000),  # Alvesta -> Vaxjo
    ...
    # Supply: Almhult=2000, Markaryd=750, Varnamo=750
    # Demand: Ljungby=1000, Alvesta=500, Vaxjo=2000
    num_nodes = 8
    A_eq = np.zeros((num_nodes, num_edges))
    ...
    print("\nOptimal flow:")
    for i, (from_node, to_node, cost, capacity) in enumerate(edges):
        node_names = ['Almhult', 'Markaryd', 'Varnamo', 'Liatorp', 'Osby', 'Ljungby', 'Alvesta', 'Vaxjo']
        if result.x[i] > 0.001:
            print(f"  {node_names[from_node]:10s} -> {node_names[to_node]:10s}: {result.x[i]:8.2f} units (cost: {cost}, capacity: {capacity})")
...
def solve_exercise_1_2_vislanda():
    ...
    # Nodes: Almhult(0), Markaryd(1), Vislanda(2), Liatorp(3), Osby(4), Ljungby(5), Alvesta(6), Vaxjo(7)
    ...
    # Supply: Almhult=2120, Markaryd=1000, Vislanda=380
    # Demand: Ljungby=1000, Alvesta=500, Vaxjo=2000
    num_nodes = 8
    A_eq = np.zeros((num_nodes, num_edges))
    ...
    print("\nOptimal flow:")
    for i, (from_node, to_node, cost, capacity) in enumerate(edges):
        node_names = ['Almhult', 'Markaryd', 'Vislanda', 'Liatorp', 'Osby', 'Ljungby', 'Alvesta', 'Vaxjo']
        if result.x[i] > 0.001:
            print(f"  {node_names[from_node]:10s} -> {node_names[to_node]:10s}: {result.x[i]:8.2f} units (cost: {cost}, capacity: {capacity})")
