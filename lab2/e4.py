...
def penalty_method(x0, mu0=0.1, beta=10, tol=1e-4, max_iter=100):
    ...
    print("=" * 80)
    print(
        f"{'Iter':<6} {'mu':<12} {'x':<25} {'f(x)':<12} {'alpha(x)':<12} {'Feasible':<10}"
    )
    print("-" * 80)
    ...
    while iterations < max_iter:
        ...
        print(
            f"{iterations:<6} {mu_k:<12.2f} ({x_k[0]:6.4f}, {x_k[1]:6.4f})    {f_k:<12.6f} {alpha_k:<12.6e} {str(feasible):<10}"
        )
...
if __name__ == "__main__":
    ...
    for res in results:
        status = (
            "CONVERGED"
            if res["iteration"] == len(results) - 1 and res["alpha"] < 1e-4
            else ""
        )
        print(
            f"Iteration {res['iteration']}: mu={res['mu']:.2e}, "
